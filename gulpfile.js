...
// Watch
gulp.task('stream', () => {
  gulp.watch(gulp.sources.src + '/views/**/*.html', gulp.series('fileincludedev'));
...
// Minify CSS, JS
gulp.task('minify', () => {
  return gulp.src(gulp.sources.dist + '/*.*')
    .pipe(useref())
...
gulp.task('run:dev',()=>{
  gulp.series('clean')(gulp.parallel([ 'connect:dev',  'imagedev', 'fileincludedev', 'scripts_dev', 'sass', 'stream', 'copy:fonts_dev'], 'browser-sync', () => {
